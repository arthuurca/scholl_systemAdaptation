...
> **Status Atual:** Em adaptação arquitetural com implementação de padrões de projeto (Design Patterns) para melhorar a escalabilidade e manutenibilidade do código.
...
A aplicação foi adaptada para utilizar o padrão **Singleton** no gerenciamento da conexão com o banco de dados SQLite. 

**Benefícios desta implementação:**
- **Uma única conexão**: Evita múltiplas conexões simultâneas, economizando recursos
- **Thread-safe**: Seguro para uso em ambiente Flask com requisições simultâneas
- **Melhor performance**: Reutiliza a mesma conexão em toda a aplicação
- **Consistência**: Garante uma única fonte de verdade para acesso aos dados
...
Veja `singleton_database.py` para a implementação completa.
